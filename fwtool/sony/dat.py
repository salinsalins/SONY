...
def readChunks(file):
 ...
 while True:
  ...
  ###if offset >= os.path.getsize(file.name):
    break
...
def readDat(file):
 ...
 for ch in chunks:
  print(ch, 'offset:', chunks[ch].offset, 'size:', chunks[ch].size)

 datv = DatvChunk.unpack(chunks[datvChunkType])
 print('DATV: data version', datv.dataVersion, '(', datvDataVersion, 'expected)')
 ...
 print('PROV: protocol version', prov.protocolVersion, '(', datvDataVersion, 'expected)')
 ...
 print('UDID: ', len(descriptors), 'descriptors')

 dend = DendChunk.unpack(chunks[dendChunkType])
 print('DEND: checksum', dend.crc, '(', _calcCrc(file, fileSize), 'calculated)')
 if _calcCrc(file, fileSize) != dend.crc:
  raise Exception('Wrong datfile checksum')
