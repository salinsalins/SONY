...
class AesCbcCrypter(AesCrypter):
 ...
 def encryptBlock(self, data):
  if self.isFirstBlock:
   self._cipher2 = AES.new(self._key, AES.MODE_CBC, self._iv)
   return super(AesCbcCrypter, self).encryptBlock(data[:512]) + self._cipher2.encrypt(data[512:])
  else:
   return self._cipher2.encrypt(data)
 ...
 def encrypt(self, file):
  ...
  #file2 = FilePart(file1, 0, size)
  file1.seek(-0x110, 2)
  self._rest = file1.read(0x110)
  file1.close()
  result = super(AesCbcCrypter, self).encrypt(file)
  return result
